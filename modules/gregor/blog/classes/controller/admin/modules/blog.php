...
class Controller_Admin_Modules_Blog extends Controller_Admin_Front {
	...
	public $template = 'modules/blog/list';
	...
	public function action_index()
	{
		...
		$this->sub_title = __('Posts list');
		$this->template
			->set_filename('modules/blog/list')
	...
	public function action_edit()
	{
		...
		$wrapper = ORM_Helper::factory('post');
		if ( (bool) $id) {
			$wrapper->orm()
				->where('id', '=', $id)
				->find();
			...
			$this->sub_title = __('Edit post');
		} else {
			$this->sub_title = __('Add post');
		}
...
// 				Controller_Admin_Structure::clear_structure_cache();
		...
		if ( ! empty($errors) OR $submit != 'save_and_exit') {
			$this->template
				->set_filename('modules/blog/edit')
				->set('errors', $errors)
				->set('wrapper', $wrapper);
		} else {
			Request::current()->redirect( $list_url );

...
class Kohana_Image_GD extends Image {
	 ...
	 * @return  void
	 */
	protected function _do_truncate($color, $distance, $top = NULL, $right = NULL, $bottom = NULL, $left = NULL)
	{
		$color_index = imagecolorexact( $this->_image, $color[0], $color[1], $color[2]);
		...
		{
			$tmp_height = $height - 1;
			while ( $top_border < $tmp_height )
			{
				$count = 0;
				for( $i = 0; $i < $width; $i++ )
				{
					if ( self::colordist( imagecolorat($this->_image, $i, $top_border), $color_index ) > $distance )
					{
						$count++;
					}
				}
		...
		if ($bottom)
		{
			while ( $bottom_border > 0 )
			{
				...
				for( $i = 0; $i < $width; $i++ )
				{
					if ( self::colordist(imagecolorat($this->_image, $i, $bottom_border), $color_index) > $distance )
					{
						$count++;
					}
				}
		...
		{
			$tmp_width = $width - 1;
			while ( $left_border < $tmp_width )
			{
				$count = 0;
				for( $i = 0; $i < $height; $i++ )
				{
					if ( self::colordist( imagecolorat($this->_image, $left_border, $i), $color_index) > $distance )
					{
						$count++;
					}
				}
		...
		if ($right)
		{
			while ($right_border > 0){
				...
				for( $i =0; $i < $height; $i++ )
				{
					if ( self::colordist(imagecolorat($this->_image, $right_border, $i), $color_index) > $distance )
					{
						$count++;
					}
				}
		...
		$this->_do_crop($new_width, $new_height, $left_border, $top_border);
